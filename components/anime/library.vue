<template>
  <div id="library" class="flex-inline flex-wrap">
    <div v-if="!pending" :class="pending ? 'animate-pulse' : ''"
      ...
      <div v-for="item in packages.content" :key="item.id"
        ...
        <div class="p-5">
          ...
          <NuxtLink class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
             :to="`/package/${item.id}`">
            Read more
    ...
    <div v-if="totalPage === 0 && !pending" class="flex items-center justify-center h-screen">
      <not_found></not_found>
    </div>
