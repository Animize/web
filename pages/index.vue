<template>
  <div class="flex flex-col">
    <div v-if="!bannerPending" class="item w-full items-center flex flex-col md:flex-row text-left">
      <Carousel
          :autoplay="0" :breakpoints="carouselBreakpoints" :pauseAutoplayOnHover="true" :settings="carouselSetting"
          :transition="500" :wrapAround="true" class="w-full h-auto">
    ...
    <div v-if="credential" class="item flex flex-col md:flex-row">
      <div class="basis-1/2 h-96 card p-2 m-2 flex flex-col">
        <LazyCommonShimmerCard v-if="historyPending"/>
        <div v-if="!historyPending && history?.data?.totalElements > 0"
        ...
        <div v-if="!historyPending && history?.data?.totalElements > 0"
          ...
          <div v-for="hst in history?.data?.content" :key="hst?.id" class="flex flex-row p-4">
            <LazyNuxtImg :src="hst?.packages?.cover ?? '/icon/img_notfound.png'" class="basis-1/4 aspect-[7/10] w-4"
                         placeholder/>
            <div class="basis-3/4">
      ...
      <div class="basis-1/2 h-96 card p-2 m-2 flex flex-col">
        ...
        <div v-if="!favoritesPending && favorites?.data?.totalElements > 0"
             class="item relative flex items-center justify-between m-2">
          <div class="left-0 animize-text-subtitle">
        ...
        <div v-if="!favoritesPending && favorites?.data?.totalElements > 0"
          ...
          <div v-for="fvr in favorites?.data?.content" :key="fvr?.id" class="flex flex-row p-4">
            <LazyNuxtImg :src="fvr?.packages?.cover ?? '/icon/img_notfound.png'" class="basis-1/4 aspect-[7/10] w-4"
                         placeholder/>
            <div class="basis-3/4">
              <div class="animize-text-subtitle p-2 line-clamp-2">
                {{ fvr?.packages?.name }}
...
const {data: bannerPackages, pending: bannerPending, refresh: bannerRefresh} = await useLazyAsyncData(
    'bannerPackages',
    () => useAPI<ResponsePageDTO>(`${config.public.API_BASE_URL}/packages/page?page=0&size=10&sort=name,desc`)
)


const {data: history, pending: historyPending, refresh: historyRefresh} = await useLazyAsyncData(
    'homeHistory',
    () => useAPI<ResponsePageDTO>(`${config.public.API_BASE_URL}/myself/watch?page=0&size=5&sort=lastModifiedDate,desc`),
    {
      immediate: false,
      watch: [
        credential
...
const {data: favorites, pending: favoritesPending, refresh: favoritesRefresh} = await useLazyAsyncData(
    'homeFavorites',
    () => useAPI<ResponsePageDTO>(`${config.public.API_BASE_URL}/myself/favorites?page=0&size=5&sort=createdDate,desc`),
    {
      immediate: false,
      watch: [
        credential
